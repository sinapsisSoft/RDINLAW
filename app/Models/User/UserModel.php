...
class UserModel implements IModel
{
  ...
  public function __construct()
  {
    ...
    $this->data = array();
    $this->result = array();
    $this->model = ['User_id', 'User_name','User_identification','User_email', 'User_title', 'User_telephone', 'Stat_id', 'Role_id', 'Sgroup_id','Comp_id','User_user','User_password','User_status_FK','created_at','updated_at'];
  }
  ...
  public function spLogin(array $data)
  {
    try {
      $this->conn =  $this->objConn->Connected();
      $this->sql = 'CALL sp_login_user(' . $data[$this->model[10]] . ')';
      $this->result = $this->conn->query($this->sql);
  ...
  public function spForgotPassword(array $data)
  {
    try {
      $this->conn =  $this->objConn->Connected();
      $this->sql = 'CALL sp_show_user(' . $data[$this->model[10]] . ')';
      $this->result = $this->conn->query($this->sql);
      $getRow = [];
      while ($row = $this->result->fetch_assoc()) {
        array_push($getRow, $row);
